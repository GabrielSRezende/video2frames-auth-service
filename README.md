...
Esse serviço não participa do pipeline de processamento em si, não acessa S3, SQS ou fila nenhuma. O papel dele é só autenticar usuários e emitir os JWTs (assinados com HS256, segredo compartilhado) que o `video-service` valida para proteger a API de upload.

A arquitetura interna segue o modelo **hexagonal (ports & adapters)**, dividida em três camadas:
...
- `infrastructure`: adapters concretos, como REST (`AuthController`), persistência JPA/Postgres e segurança (Spring Security + JJWT).
...
**Fluxos principais:**
- **Registro**: `AuthController.register` chama `RegisterUserUseCase`, que valida se o e-mail já existe (`EmailAlreadyRegisteredException` se sim), cria o `User` com senha já hasheada via `BCryptPasswordHasher` e persiste.
- **Login**: `AuthController.login` chama `LoginUseCase`, que busca o usuário por e-mail, valida a senha (`InvalidCredentialsException` se inválida) e delega ao `TokenIssuer` a emissão do par access/refresh token.
- **Refresh**: `AuthController.refresh` chama `RefreshTokenUseCase`. Ele valida o refresh token opaco comparando o hash via `Sha256TokenHasher`, revoga o token usado (rotação) e emite um novo par pelo `TokenIssuer`.
...
Os endpoints do Actuator (`/actuator/health`, `/actuator/info`, `/actuator/prometheus`, `/actuator/metrics`) também ficam liberados sem autenticação, veja a seção [Observabilidade](#observabilidade--monitoramento).

## Stack tecnológica

- **Java 17**
- **Spring Boot 4.1** (Web, Security, Data JPA, Validation, Actuator)
- **Spring Security**, com filtro JWT stateless (`JwtAuthenticationFilter`) e BCrypt para hash de senha
...
Útil no dia a dia de desenvolvimento: dá pra ter hot-reload e debugar direto na IDE, deixando só o Postgres containerizado.
...
O serviço usa o **logging estruturado nativo do Spring Boot 4** (sem dependências extras), o que permite alternar o formato de saída só trocando uma variável de ambiente:

- `LOG_FORMAT` vazio (padrão) → logs em texto simples no console, bons para desenvolvimento local.
- `LOG_FORMAT=ecs` → logs em **JSON no formato ECS** (Elastic Common Schema), prontos para ingestão direta em **AWS CloudWatch Logs Insights** ou em uma stack **ELK**. Não muda nada no código, só a variável entre dev e staging/produção.
...
- **WARN** para falhas esperadas do domínio (credenciais inválidas, e-mail duplicado, refresh token inválido/expirado, JWT malformado). Não viram `ERROR` porque não indicam bug, e sim uso incorreto ou tentativa maliciosa.

Nenhum log grava senha, hash de senha, JWT ou refresh token em texto puro, só identificadores não sensíveis (e-mail, id de usuário).
...
Para visualização, o repositório irmão **`video2frames-infra-ops`** mantém um `docker-compose` com **Prometheus** (que faz *scrape* de `/actuator/prometheus` dos 4 microserviços via `host.docker.internal`) e **Grafana**, já com um dashboard pré-provisionado ("Video2Frames - Overview"). É só subir aquele stack separadamente para acompanhar as métricas deste serviço em tempo real.
